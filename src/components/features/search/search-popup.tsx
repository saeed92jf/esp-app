...
export function SearchPopup<T>({
  ...
  // Lock body scroll when the search dropdown is open.
  // Compensate for the scrollbar width to prevent layout shift.
  useEffect(() => {
    ...
    if (shouldShowDropdown) {
      ...
      // In RTL (Persian), scrollbar is on the left, so use paddingInlineStart
  ...
  return (
    <>
      {/* Full-screen backdrop: blurs and darkens the page behind the search popup.Rendered in a portal-like fashion outside the relative wrapper so it covers
          the entire viewport. Pointer events are disabled so clicks pass through to
      ...
      {shouldShowDropdown && (
        <div
          ...
          // Clicking the backdrop dismisses the search
          onMouseDown={() => {
            setIsFocused(false);
            setQuery("");
          }}
      ...
      <div
        ...
        style={{
          zIndex: shouldShowDropdown ? "var( --z-search)" : undefined,
        }}
