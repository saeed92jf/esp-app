...
> = {
  ...
  operatorNode: { width: 160, height: 96 },
  geometryCalcNode: { width: 220, height: 340 },
  beamCalcNode: { width: 240, height: 360 },
  shapeNode: { width: 200, height: 300 },
...
export function getShapeGeometry(
  ...
  switch (shape) {
    ...
    case "delayNode": {
      // "D" shape â€” flat left edge, fully rounded right edge.
      const r = h / 2 - inset;
      const path =
        `M ${inset},${inset} ` +
        `L ${w - r - inset},${inset} ` +
    ...
    case "numberNode":
    case "operatorNode":
      // Rendered by their own components (NumberNode/OperatorNode), not through ShapeCanvas/SVG.
      return { kind: "none" };
