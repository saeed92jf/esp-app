// src/modules/esp-flow/utils/validation.ts
// ─────────────────────────────────────────────────────────────────────────
// Connection rules used by <ReactFlow isValidConnection={...}>.
// https://reactflow.dev/examples/interaction/validation
//
// Per-handle connection-count limits were removed entirely — for ordinary
...
export function makeIsValidConnection(getState: () => { nodes: Node[]; edges: Edge[] }) {
  return (connection: Connection | Edge): boolean => {
    ...
    // Rule 1 — no self-loops (a node connecting to itself).
    if (source === target) return false;

    // Rule 2 — no exact duplicate edges (same source+target+handles already connected).
